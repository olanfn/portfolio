...
const socials = [
  {
    name: 'LinkedIn',
    href: 'https://linkedin.com/in/rolanok',
    icon: (
  ...
  {
    name: 'Instagram',
    href: 'https://instagram.com/rolanok',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
    ...
    color: '#E4405F',
    bg: 'hover:bg-gradient-to-r hover:from-[#833ab4] hover:via-[#fd1d1d] hover:to-[#fcb045]',
  },
  {
    name: 'TikTok',
    href: 'https://tiktok.com/@rolanok',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
    ...
    color: '#000',
    bg: 'hover:bg-black dark:hover:bg-gray-900',
  },
  {
    name: 'Facebook',
    href: 'https://facebook.com/rolanok',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
...
export default function About() {
  ...
  return (
    <section id="about" ref={sectionRef} className="py-24 bg-gray-50/50 dark:bg-[#0A0A0F]">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
                ...
                {socials.map((social) => (
                  <a
                    ...
                    className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-sm font-medium 
                      bg-gray-100 dark:bg-[#1A1A2E] text-gray-700 dark:text-gray-300
                      border border-gray-200 dark:border-[#2A2A4A]
                      ${social.bg} hover:text-white hover:border-transparent
          ...
          <div className="space-y-6">
            {Object.entries(skills).map(([category, skillList]) => (
              <div key={category} className="reveal bg-white dark:bg-[#1A1A2E] rounded-2xl p-6 border border-gray-200 dark:border-[#2A2A4A]">
                <h4 className="font-grotesk font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-gradient-to-r from-[#6C63FF] to-[#00D4FF]" />
                  {category}
                </h4>
                <div className="flex flex-wrap gap-2">
                  {skillList.map((skill) => (
                    <span
                      key={skill}
                      className="px-3 py-1.5 rounded-lg text-xs font-medium bg-gray-50 dark:bg-[#0A0A0F] 
                        text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-[#2A2A4A]
                        hover:border-[#6C63FF]/40 hover:text-[#6C63FF] dark:hover:text-[#6C63FF] transition-colors"
                    >
                      {skill}
                    </span>
                  ))}
